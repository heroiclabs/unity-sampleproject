...
namespace DemoGame.Scripts
{
    ...
    public class NakamaSessionManager : Singleton<NakamaSessionManager>
    {
        ...
        /// </summary>
        [SerializeField] private int _reauthentication = 5;
        ...
        /// <summary>
        /// If true, this client is connected to Nakama server using their Facebook account.
        ...
        /// </summary>
        private IApiAccount Account { get; set; }
        ...
        /// </summary>
        public bool IsConnected => Session != null && Client != null;
        ...
        public bool IsFacebookConnected
        {
            ...
            private set
            {
                ...
                PlayerPrefs.SetInt("nakama.facebookAuthentication", _isFacebookConnected ? 1 : 0);
            }
        }
        ...
        /// Read logs for more information about the error when it occurs.
        /// </summary>
        ...
        /// upon connection re-authentication.
        /// </summary>
        public event NakamaSessionDelegate OnAuthenticationSuccess =
            delegate(Client client, ISession session)
            {
                Debug.Log(">> Authentication Success; session: " + session.AuthToken);
            };
        ...
        /// </summary>
        public event NakamaSessionDelegate OnDeviceIdAccountCreated = delegate
        {
            Debug.Log(">> New DeviceId Account");
        };

        /// <summary>
        /// Invoked upon logging with new Facebook account.
        /// </summary>
        public event NakamaSessionDelegate OnFacebookAccountCreated = delegate
        {
            Debug.Log(">> New Facebook Account");
        };
        ...
        /// <summary>
        /// If current <see cref="Session"/> has expired, reauthenticate the connection with
        ...
        public async void Reauthenticate(bool force)
        {
            ...
            if (!force && Session.HasExpired(DateTime.UtcNow) != true) return;
            if (IsFacebookConnected)
            {
        ...
        /// <summary>
        /// Coroutine forcing connection to reauthenticate with Nakama server after some delay based on current time,
        ...
        private IEnumerator ReauthenticateDelayCoroutine()
        {
            ...
            while (Session != null)
            {
                var secondsUntilExpiration = Session.ExpireTime - DateTime.UtcNow.TotalSeconds();
                yield return new WaitForSeconds(secondsUntilExpiration - _reauthentication);

                var lastSession = Session;
                Reauthenticate(true);
                // Waiting until Reauthenticate method resolves asynchronously and updates Session object with new AuthToken
        ...
        private void GetPlayerPrefs()
        {
            ...
            var facebookAuthentication = PlayerPrefs.GetInt("nakama.facebookAuthentication", 0);
            IsFacebookConnected = facebookAuthentication == 1;
        }
        ...
        private void InitializeFacebook(NakamaSessionDelegate onSuccess, Action onFailure)
        {
            ...
            FB.LogInWithReadPermissions(permissions,
                result => FacebookInitialized(result, onSuccess, onFailure));
        }
        ...
        /// </summary>
        private async Task AuthenticateFacebookAsync(IResult result = null)
        {
            if (FB.IsLoggedIn)
            {
                var newAccountCreated = false;
